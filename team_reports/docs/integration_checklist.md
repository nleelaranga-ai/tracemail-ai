...
**Component:** Reports Engine (Team Reports)  
...
> Member 2's Backend retrieves data from PostgreSQL, aggregates findings from AI Engine (Member 3), Threat Intel (Member 4), and Maps (Member 5), and passes the assembled payload to Team Reports.
...
Once mounted, Team Reports exposes three endpoints:
...
1. **Step 1: Install Requirements**
   Add Team Reports requirements to your environment:
   ```bash
   pip install -r team_reports/requirements.txt
   ```
...
3. **Step 3: Direct In-Process Calling (Alternative to HTTP)**
   ...
   ```python
   from team_reports.reports.json.json_report import JSONReportGenerator
...
4. **Step 4: Response Headers**
   When serving the PDF file, Team Reports sets these headers for client verification:
...
```bash
pytest team_reports/tests/integration/test_reports_api.py -v -m "not pdf"
```
