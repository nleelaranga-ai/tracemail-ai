...
def seed_database():
    ...
    try:
        ...
        if existing_1:
            for k, v in inv_1_data.items():
                setattr(existing_1, k, v)
            logger.info(f"Updated demo investigation with honest provenance: {inv_id_1}")
        else:
            db.add(Investigation(id=inv_id_1, **inv_1_data))
        ...
        if existing_2:
            for k, v in inv_2_data.items():
                setattr(existing_2, k, v)
            logger.info(f"Updated demo investigation with honest provenance: {inv_id_2}")
        else:
            db.add(Investigation(id=inv_id_2, **inv_2_data))
        ...
        if existing_3:
            for k, v in inv_3_data.items():
                setattr(existing_3, k, v)
            logger.info(f"Updated demo investigation with honest provenance: {inv_id_3}")
        else:
            db.add(Investigation(id=inv_id_3, **inv_3_data))
