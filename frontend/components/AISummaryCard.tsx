...
export function AISummaryCard({
  ...
  const defaultReasons = isSafe
    ? [
        "Sender domain and sending server transmission alignment verified.",
        "Zero coercive, threatening, or artificial urgency triggers detected in email body.",
        "Zero deceptive credential harvesting links or malicious attachments identified."
      ]
    : [
        "Mismatched Sender Header: Discrepancy observed between claimed sender and envelope transmission relay.",
        "High-risk intent or deceptive hyperlink pattern flagged in message payload."
      ];
    ...
    <div className="rounded-xl border border-bg-border bg-gradient-to-b from-bg-raised to-bg p-6 shadow-lg">
      ...
      <div className="mt-5">
        <h4 className="text-xs font-semibold uppercase tracking-wider text-ink-muted">
          {isSafe ? "Verified Authenticity Indicators & Rationales" : "Detected Threat Indicators & Rationales"}
        </h4>
        <ul className="mt-3 space-y-2.5">
          {reasons.map((reason, idx) => (
            <li key={idx} className="flex items-start gap-2.5 text-xs text-ink">
              {isSafe ? (
                <CheckCircle className="mt-0.5 h-3.5 w-3.5 flex-none text-emerald-400" />
              ) : (
